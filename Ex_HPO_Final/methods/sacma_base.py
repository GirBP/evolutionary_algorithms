"""SA-CMA Base — Базова версія (Тільки Random Forest + CMA-ES)
Без фільтрації, без адаптації популяції, без парсимонії.
"""
import numpy as np
from sklearn.ensemble import RandomForestRegressor

def _cma_on_surrogate(surrogate, X_hist, y_hist, dim, rng, n_gens=50, lam=10):
...
def run(seed, obj_fn, dim, budget):
    ...
    while ec < budget:
        ...
        # CMA-ES Surrogate
        candidate = _cma_on_surrogate(rf, X_hist, y_hist, dim, rng, n_gens=50, lam=10)
