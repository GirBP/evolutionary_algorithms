"""SACMA-DAC — Surrogate-Assisted CMA-ES with Dynamic Adaptive Control"""
import numpy as np
...
def run(seed, obj_fn, dim, budget):
    ...
    while ec < budget:
        ...
        # Surrogate Filtering (масовка 200 точок)
        virtual_pop = rng.random((200, dim))
        all_cand = np.vstack([cma_best.reshape(1, -1), virtual_pop])
