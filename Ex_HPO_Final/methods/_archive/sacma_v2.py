"""SA-CMA v2 — Parsimony + Surrogate Filtering (200 random points)"""
import numpy as np
...
def _cma_on_surrogate(surrogate, X_hist, y_hist, dim, rng, n_gens=50, lam=10, parsimony_beta=0.1):
    ...
    # Parsimony heuristic
    def complexity(x): return np.mean(x)
...
def run(seed, obj_fn, dim, budget):
    ...
    while ec < budget:
        ...
        cma_candidate = _cma_on_surrogate(rf, X_hist, y_hist, dim, rng, n_gens=50, lam=10, parsimony_beta=beta_p)
        
        # Surrogate Filtering (200 random search points as global explorer)
        virtual_pop = rng.random((200, dim))
        all_cand = np.vstack([cma_candidate.reshape(1, -1), virtual_pop])
        
        p_fits = rf.predict(all_cand)
        # Apply parsimony to filter
        def cpl(x): return np.mean(x)
        p_fits = np.array([bf + beta_p * cpl(x) for bf, x in zip(p_fits, all_cand)])
