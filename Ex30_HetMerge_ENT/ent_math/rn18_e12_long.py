#!/usr/bin/env python3
"""E15: EXACT E12 approach + 1200s budget.

E12 recipe (4/10 in 7ep):
- Stage 1: FC on frozen ImageNet backbone (200ep, 10s)
- Stage 2: Full fine-tune backbone=0.0001 / fc=0.001 (as many epochs as budget allows)
- Loss: KD(s[0:5], tA, T=4) + KD(s[5:10], tB, T=4)  (w=1,1)
- Eval every epoch, save best

NO CHANGES to E12 except budget.
"""
...
# ═══ Stage 1: FC on frozen backbone (EXACT E12) ═══
...
# ═══ Stage 2: Full fine-tune (EXACT E12 params) ═══
print(f"\n--- Stage 2: Full fine-tune (EXACT E12) ---", flush=True)
torch.manual_seed(SEED)
student = make_rn(10)
...
for ep in range(15):
    ...
    print(f"  ep {ep+1}: ret={ret}/10 (A={a_ret}/5 B={b_ret}/5) mean_drop={np.mean(list(drops.values())):.1f}% ({time.time()-t0:.0f}s)", flush=True)
